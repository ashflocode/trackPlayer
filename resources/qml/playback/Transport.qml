...
Item {
    ...
    function eject() {
		...
		delay.reset()
		filter.reset()
		track_loader.trackOnDeck =
			keyboard_controls.focus =
				playback.isPlaying =
					waveforms.visible =
						edit.enabled = false
		file_name.text = playback_length.text = ""
		Controller.tracklist.metadataFields.forEach((label) => {
			metadata_window.metadataText[label].text = ""
		});
        artwork.source = "qrc:/images/artwork"
        select_track_overlay.visible = true
    }
