...
FileName::FileName(const char *name, IOStream *stream)
	: utf8FileName(name)
	, fileStream(stream)
{
}
...
FileName::FileName(const FileName &name)
        : utf8FileName(name.utf8FileName)
...
IOStream* FileName::stream()
{
	return fileStream;
}

const std::string& FileName::utf8Name() const
{
	return utf8FileName;
}
...
#if defined(_WIN32) && !defined(TAGLIB_USE_EXTERNAL_FILE_IO)
