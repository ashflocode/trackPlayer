...
void MPEG::Properties::read(File *file)
{
  if(file->containerFormat() == File::ADTS) {
    ...
    d->channels          = firstHeader.channelMode() == AdtsHeader::FrontCenterLeftRightSideLeftRightBackLeftRightLFE ? 8 : static_cast<int>(firstHeader.channelMode());
    d->version           = firstHeader.version();
    d->layer             = 0;
    d->protectionEnabled = firstHeader.protectionEnabled();
    d->channelMode       = firstHeader.channelMode() == AdtsHeader::FrontCenter ? Header::SingleChannel : Header::JointStereo;
